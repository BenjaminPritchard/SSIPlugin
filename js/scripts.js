    ...
    if (state === "Issued") {
      ...
      $.ajax({
        ...
        data: {
          ...
          arguments: [identifier, credentialId, role],
        },
        success: function (result) {
          console.log(result);
          $(".qrcode").hide();
...
function pollingAuth(verificationId) {
  let refreshId = setInterval(function () {
    ...
    if (state === "Accepted" && isValid === true) {
      console.log(state);
      identifier =
        jsonResult["proof"]["credential"]["attributes"]["Identifier"];
      $.ajax({
        url: urlAjax,
        type: "POST",
        ...
        success: function (result) {
          console.log(result);
          $(".qrcode").hide();
          //fixme; this is a hard-coded hack
          window.location.href = "https://www.benjaminpritchard.org/videos/";
        },
      });
      clearInterval(refreshId);
...
function createAndOfferCredential(identifier, role) {
  $.ajax({
    ...
    type: "POST",
    data: {
      functionname: "createAndOfferCredential",
      arguments: [identifier, role],
    },
    success: function (result) {
      try {
        jsonResult = JSON.parse(result);
